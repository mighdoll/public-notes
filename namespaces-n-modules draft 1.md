...
- Libraries arrive as sets of files, each with its own view of its dependencies. The API takes dependencies alongside sources: `createShaderModule({ sources, dependencies: { lygia, noise } })`. Two libraries that want different versions of `noise` each get their own. The package manager decides which dependencies are shared and which aren't, and hands the browser the result, with no library text rewritten.
