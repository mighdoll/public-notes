Naming and addressing code units is a fine direction for WebGPU/WGSL!
...
Either way, WGSL gets a way to address code in other code units, e.g. `foo::bar()`, and a `wgsl::` prefix for builtins. 
Modules need an addition to `createShaderModule()` to accept labeled code strings. Namespaces need new WGSL syntax for the namespace blocks.
...
- Big projects have many pipelines and several device configurations, each wanting a different subset of the code. With concatenation, the user or a tool has to select that subset first. Otherwise, an unused file containing `enable f16;` can make shader creation fail on a device without `shader-f16` enabled. With modules, we can specify that the browser follows references from a chosen main source, parsing and validating only the sources reached that way. dneto0 asked how a translator skips syntax it doesn't understand [here](https://github.com/gpuweb/gpuweb/issues/5140#issuecomment-3293396567). Presuming WGSL gets conditions someday, with modules the answer is that unreferenced modules are not parsed.
...
- Files are the units. No wrapper and no name to keep in sync; the directory tree is the hierarchy, and errors point at files.
- The browser compiles from the main module and follows references, so each pipeline gets exactly the code it needs, and an unreferenced module a device can't use is never parsed, rather than fatal.
- Libraries can be just sets of files, each with its own view of its dependencies. The API takes dependencies alongside sources, each dependency with its own sources and dependencies: `createShaderModule({ sources, dependencies: { lygia, noise } })`. Two libraries that want different versions of `noise` each get their own. The package manager decides which dependencies are shared and which aren't, and hands the browser the result, with no library text rewritten.
