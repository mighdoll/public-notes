...
I want to push on one design choice for the draft: where to declare the name of a code unit. Defining terms for the following discussion:
...
Either way, WGSL gets a way to address code in other code units, e.g. `foo::bar()`, and a `wgsl::` prefix for builtins. The implementations inside WebGPU should be similar overall. 
Modules need a small addition to `createShaderModule()` to accept labeled code strings,
namespaces need new WGSL syntax for the namespace blocks.

They're similar, but start with modules. Moving the labels to the outside leads to a better place for users: a browser native standard for code sharing, with no external rewriting/linking tools required.

Using namespaces alone isn't ideal for bigger projects that want to organize their code:
- The directory tree already gives every file a unique name (`render/util.wgsl` vs `physics/util.wgsl`). Namespaces make users write that hierarchy again, inside each file, and keep the two in sync. It's a maintenance and readability burden.
- Translator error line numbers point into the concatenated code blob, rather than the files. Source maps (#4844) for shaders in browsers will help, but relying on them
  makes source maps foundational for everyone, not just for transpiled shader languages.
...
Libraries make it worse. A library is just a subsystem maintained by people with whom you can't coordinate naming:
- Namespaces plus concatenation create collisions once two libraries share a dependency.  Say the app uses `libA` and `libB`, and both ship a copy of `namespace noise`.  Concatenating includes `noise` twice: without reopening that's a duplicate namespace error,  with reopening every function in it is defined twice.  (C++ dedups by wrapping every header in an `#ifndef` guard. But WGSL has no preprocessor.) Someone has to dedup, by hand or with a tool, and dedup can't help when the two copies are different versions of `noise`. Then there's no fix w/o rewriting the text, so a package manager can't just deliver a bundle of code plus its dependencies.
...
- Libraries arrive as sets of files, each with its own view of its dependencies. The package manager decides which dependencies are shared and which aren't, and hands the browser the result, with no library text rewritten. (presuming we'd extend the api above a notch further)

Basically, a browser standard so users can share shader code would be great! Namespaces alone don't give us that, but modules do.
