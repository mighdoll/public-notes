...
Our sense is that most widely useful language 'ergonomic' features, from a module system to reflection to generics are in scope for potential browser implementation. Is that right? Clarity that e.g. modules are in scope for WebGPU/WGSL eventually would help users, e.g. in [gpuweb#5456](https://github.com/gpuweb/gpuweb/issues/5456). Our current stance is that any WESL feature that _could_ be upstreamed to browser WebGPU/WGSL _should_ be designed so that it can upstreamed. Let us know if that's wrong for any features.
...
- Is this user/task class definition the right way to add some new features w/o excessive complexity for every WebGPU user? What design assumptions / use targets do you think about? Are the salient differences between WebGPU uses and users that are different from other shader language communities?
#### Module system
...
# WESL Requests for WebGPU features
